                      ...
                      {viewTab === "questions" ? (
                        ...
                        <div className="space-y-6">
                          {!canReadAggregates && !canReadRaw ? (
                            ...
                            <div className="flex justify-center rounded-xl border border-slate-200 bg-white py-12" role="status">
                            ...
                            <div className="space-y-6">
                              <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-200 bg-white p-3">
                                <p className="text-xs text-slate-500">
                                  {canReadRaw ? "Raw response access is enabled." : "Showing privacy-preserving aggregates only."}
                                </p>
                                <div className="flex flex-wrap gap-2">
                                  {canExport && (
                                    <Button variant="outline" size="sm" onClick={() => void handleExportResponses(getSurveyResponseResourceId(survey))} disabled={responseAction !== null}>
                                      {responseAction === "export" ? <Loader2 className="animate-spin" data-icon="inline-start" /> : <Download data-icon="inline-start" />}
                                      Export responses
                                    </Button>
                                  )}
                                  {canErase && canReadRaw && selectedResponseIds.length > 0 && (
                                    <Button variant="destructive" size="sm" onClick={() => void handleEraseResponses(survey, "selected")} disabled={responseAction !== null}>
                                      Erase selected
                                    </Button>
                                  )}
                              ...
                              {canReadRaw && surveyResponses.length > 0 && (
                                <div className="rounded-xl border border-slate-200 bg-white p-4">
                                  <div className="mb-3 flex items-center justify-between gap-2">
                                    <h5 className="text-sm font-semibold text-slate-800">Raw response records</h5>
                                    {canErase && <span className="text-xs text-slate-500">Select records to erase</span>}
                                  </div>
                                  <div className="max-h-48 space-y-1 overflow-y-auto">
                                    {surveyResponses.map((response) => (
                                      <label key={response.id} className="flex items-center gap-2 rounded-md px-2 py-1.5 text-xs text-slate-600 hover:bg-slate-50">
                                        {canErase && (
                                          <input
                                            type="checkbox"
                                            aria-label={`Select response ${response.id}`}
                                            checked={selectedResponseIds.includes(response.id)}
                                            onChange={(event) => setSelectedResponseIds((current) => event.target.checked
                                              ? [...current, response.id]
                                ...
                                {survey.sections?.map((sec, secIdx) => (
                                  <div key={secIdx} className="space-y-5">
                                    ...
                                    {sec.questions?.map((q, qIdx) => {
                                        ...
                                        return (
                                          <div key={label} className="group">
                                            ...
                                            <div className="h-3 w-full overflow-hidden rounded-full bg-slate-100/80 shadow-inner">
                                              <div className={cn("h-full rounded-full transition-all duration-500", index === 0 ? "bg-indigo-500 shadow-sm" : "bg-indigo-300 group-hover:bg-indigo-400")} style={{ width: `${percent}%` }} />
                                            </div>
                                          </div>
                                        )
                                      }
                                      ...
                                      return (
                                        <div key={q.id || qIdx} className="rounded-xl border border-slate-200/80 bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
